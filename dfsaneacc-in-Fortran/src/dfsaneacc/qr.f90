module bmqr
  ...
  public :: qr, extractQ, extractR, qrupdate, qrsolve
...
contains
  ...
  subroutine qr(n,m,lda,A,gam,p,rank)
    ...
    real(kind=8), intent(inout) :: A(lda,m)
    real(kind=8), intent(out) :: gam(m)
    
    ! PARAMETERS
    real(kind=8) :: tol = 100.0d0 * epsilon( 1.0d0 )
    
    ! LOCAL SCALARS
    integer :: imax,itmp,j,k,kmax
    real(kind=8) :: tau,tmp
    
    ! LOCAL ARRAYS
    real(kind=8) :: c(m),cnrm(m+1),cnrmfull(m+1),v(n)
  ...
  subroutine extractQ(n,m,lda,A,gam,rank,ldq,Q)

    implicit none

    ! SCALAR ARGUMENTS
    integer, intent(in) :: lda,ldq,m,n,rank
    
    ! ARRAY ARGUMENTS
    real(kind=8), intent(in) :: A(lda,m),gam(m)
    real(kind=8), intent(out) :: Q(ldq,m)
  ...
  subroutine extractR(m,lda,A,rank,ldr,R)

    implicit none

    ! SCALAR ARGUMENTS
    integer, intent(in) :: lda,ldr,m,rank
    
    ! ARRAY ARGUMENTS
    real(kind=8), intent(in) :: A(lda,m)
    real(kind=8), intent(out) :: R(ldr,m)
  ...
  subroutine qrupdate(n,m,rank,mlim,jtbr,ldq,Q,ldr,R,p,newcol)

    implicit none

    ! SCALAR ARGUMENTS
    integer, intent(in) :: jtbr,ldq,ldr,mlim,n
    integer, intent(inout) :: m,rank
  
    ! ARRAY ARGUMENTS
    integer, intent(inout) :: p(mlim)
    real(kind=8), intent(in), optional :: newcol(n)
    real(kind=8), intent(inout) :: Q(ldq,mlim),R(ldr,mlim)
    ...
    ! PARAMETERS
    real(kind=8) :: tolsqr = 10.0d0 * sqrt( epsilon( 1.0d0) )
  
    ! LOCAL SCALARS
    integer :: i,itmp,j,jmax,jtbrpos,k
    real(kind=8) :: cc,ss,tau,tmp
  
    ! LOCAL ARRAYS
    real(kind=8) :: v(mlim)

    if ( n .lt. m .or. rank .gt. m .or. m .gt. mlim .or. ldq .lt. n .or. ldr .lt. mlim ) then
  ...
  subroutine qrsolve(n,m,rank,ldq,Q,ldr,R,p,b,x)

    implicit none

    ! SCALAR ARGUMENTS
    integer, intent(in) :: ldq,ldr,m,n,rank
  
    ! ARRAY ARGUMENTS
    integer, intent(in) :: p(m)
    real(kind=8), intent(in) :: Q(ldq,rank),R(ldr,m),b(n)
    real(kind=8), intent(out) :: x(m)
    ...
    ! LOCAL ARRAYS
    real(kind=8) :: kap(rank),Rt(m,rank)
  ...
  subroutine prmatrix (M)
    real(kind=8), dimension(:,:) :: M

    integer :: i, j
    do i = 1, size(M,1)
       do j = 1, size(M,2)
          write (*,fmt='(F10.4)',advance='no') M(i,j)
       end do
       write (*,*)
    end do
  end subroutine prmatrix
